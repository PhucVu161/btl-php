...
<?php
  ...
  $offset = ($page - 1) * $limit;
  $sql = "SELECT * FROM dm_phieu WHERE tendm LIKE '%$search%'";
...
<?php while($row=mysqli_fetch_array($query)): ?>
                <tr>
                   ...
                   <td><a href="sua_danhmuc.php?id=<?php echo $row['id']; ?>"><button type="button" class="btn btn-success">Sửa</button></a></td>
                   <td><a href="xoa_danhmuc.php?id=<?php echo $row['id']; ?>" onclick="return confirm('Bạn có muốn xóa?')"><button type="button" class="btn btn-danger">Xóa</button></a></td>
                </tr>
             </thead>
<?php endwhile;?>
              </table>
